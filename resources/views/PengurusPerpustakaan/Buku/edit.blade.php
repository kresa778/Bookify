    ...
    <div class="main-content">
        ...
        <main class="container-fluid">
            <div class="container">
                ...
                <form action="{{ route('pengurus.buku.update', $buku->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="lokasi_buku" value="{{ $buku->lokasi_buku }}"
                                class="form-control" placeholder="Contoh: Rak A1">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="status_buku" class="form-label">Status Buku:</label>
                            <select name="status_buku" class="form-select">
                                <option value="Tersedia" {{ $buku->status_buku == 'Tersedia' ? 'selected' : '' }}>
                                    Tersedia</option>
                                <option value="Dipinjam" {{ $buku->status_buku == 'Dipinjam' ? 'selected' : '' }}>
                                    Dipinjam</option>
                                <option value="Hilang" {{ $buku->status_buku == 'Hilang' ? 'selected' : '' }}>Hilang
                                </option>
                            </select>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label for="sinopsis" class="form-label">Sinopsis:</label>
                            <textarea class="form-control" style="height:150px" name="sinopsis" placeholder="Sinopsis">{{ $buku->sinopsis }}</textarea>
                        </div>
